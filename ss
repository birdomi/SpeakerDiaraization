...
def runTrain(sess,d,rnn,msg):
    ...
    rnn.Restore('model/RNN_1_20180307172902')
	...
	##
    total_number_of_batch=3;total_number_of_batch_test=1
    num_epoch=1
    totalTime=Time()
    for curr_epoch in range(0,num_epoch):
        ...
        for number in trainRange:
            for index in range(3):
                cost,_=rnn.Train(d[number]._MFCC[index],d[number]._LABEL[index],1.0)
                cost_sum+=cost
...
sess=tf.Session()
rnn=a.RNN_Model_usingAll(sess,'RNN',0.001)
runTrain(sess,dN,rnn,rnn.name)
for number in testRange:
    for index in range(3,4):
           rnn.Show_Reuslt(dN[number]._MFCC[index],dN[number]._LABEL[index],wavPath+'/'+mfcc_path[number])
